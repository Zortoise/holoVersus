...
func simulate():
	if Animator.to_play_animation == "Spawn" and Globals.Game.get_player_node(Entity.master_ID).unique_data.nibbler_cancel > 0:
		Globals.Game.spawn_SFX("SmallSplash", "SmallSplash", Entity.position, {"facing":Entity.facing, "grounded":true}, Entity.master_ID)
		...
		Globals.Game.spawn_entity(Entity.master_ID, "Nibbler", spawn_point, {"facing" : Entity.facing})
		Globals.Game.spawn_SFX("BigSplash", "BigSplash", Entity.position, {"facing":Entity.facing, "grounded":true}, Entity.master_ID)
...
func _on_SpritePlayer_anim_started(anim_name):
	match anim_name:
		"Kill":
			var new_facing_ref = Globals.Game.get_player_node(Globals.Game.get_player_node(Entity.master_ID).target_ID).position.x - \
					Entity.position.x
