...
func _ready():
	...
	uniqueHUD = load("res://Characters/Gura/UniqHUD.tscn").instance()
			...
			match Animator.current_anim:
				"SP1[c1]Startup":
					if !Character.button_light in Character.input_state.pressed:
						...
						Character.cancel_action(Character.button_alter)
				"SP1[c2]Startup":
					if !Character.button_light in Character.input_state.pressed:
						if Animator.time == 1:
...
func capture_combinations():
	...
	Character.combination(Character.button_alter, Character.button_dash, "Sp.Dash")
	Character.ex_combination(Character.button_alter, Character.button_dash, "ExSp.Dash")
...
func rebuffer_actions(): # for when there are air and ground versions, or up/down versions
	...
	Character.rebuffer(Character.button_alter, Character.button_dash, "Sp.Dash")
...
#	Character.ex_rebuffer_trio(Character.button_alter, Character.button_up, Character.button_fierce, "ExSp.uF")
#	Character.ex_rebuffer_trio(Character.button_alter, Character.button_light, Character.button_fierce, "ExSp.H")
...
func process_move(new_state, attack_ref: String, has_acted: Array): # return true if button consumed
	...
	if Character.grounded:
		...
		if !air_atk_ref in STARTERS or !Character.test_aerial_memory(air_atk_ref):
			return false
...
#						continue # certain moves cannot be performed during cancellable recovery
		...
		Em.char_state.GRD_BLOCK: # for Specials using button_block
			if Settings.input_assist[Character.player_ID]:
				if Character.grounded and Character.button_block in Character.input_state.pressed and \
