...
#
		...
		"ExSp.L":
			if !has_acted[0]:
				if Character.can_afford_ex("aSP1[ex]"):
		...
		"ExSp.uF":
			if !has_acted[0] and !has_drill():
				if Character.can_afford_ex("aSP4[ex]"):
		...
		"ExSp.F":
			if !has_acted[0]:
				if Character.can_afford_ex("aSP5[ex]"):
...
func _on_SpritePlayer_anim_started(anim_name):
	...
	match anim_name:
		...
		"Blink", "EBlink":
			...
			var blink_dist := int(max(Character.get_stat("GRD_DASH_SPEED"), Character.get_stat("AIR_DASH_SPEED")))
...
#		"L1Startup", "L2Startup", "L3Startup", "HStartup" 
		"F1Startup", "F2Startup", "F3Startup":
