...
# warning-ignore:integer_division
	...
	match segment:
		...
		1:
			...
			target_pos.y = FMath.sin_lerp_m($Waypoints/A.position.y * FMath.S, $Waypoints/B.position.y * FMath.S, sub_time)
		...
		3:
			...
			target_pos.y = FMath.sin_lerp_m($Waypoints/B.position.y * FMath.S, $Waypoints/A.position.y * FMath.S, sub_time)
			return {"type":Em.mov_platform.MOVING, "pos":target_pos.convert_to_vec()}
