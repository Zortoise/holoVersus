...
#		viewport.BGM_credits(chosen_music_dict)
		...
		var chosen_music_dict = BGM.play_random_in_folder("CustomPlaylist", true)
		viewport.BGM_credits(chosen_music_dict)
		
	
	elif Globals.survival_level == null:
#		each character (no repeat) and the stage has an equal chance of its music folder being picked
#		afterwards, a random music is picked from the selected music folder
		...
		var chosen_music_dict = BGM.play_random_in_folder(sets[0], true)
		viewport.BGM_credits(chosen_music_dict)
