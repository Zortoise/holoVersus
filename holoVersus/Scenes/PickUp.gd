...
func init(in_item_ref: String, in_position: Vector2, aux_data: Dictionary, in_lifespan = null, in_palette_ref = null):
	...
	if UniqEntity.has_trait(Em.entity_trait.SPAWN_OFFSET) and has_node("EntityCollisionBox"):
		position -= Vector2(0, $EntityCollisionBox.rect_position.y + $EntityCollisionBox.rect_size.y)
	set_true_position()
...
#	}
	...
	if UniqEntity.has_node("DefaultCollisionBox"):
		...
		if UniqEntity.has_trait(Em.entity_trait.GROUNDED):
...
func simulate():
	...
	if !stasis:
		...
		if UniqEntity.has_trait(Em.entity_trait.GROUNDED) and !Animator.query_current(["Marker"]):
			if !is_on_ground($EntityCollisionBox):
				
				if UniqEntity.has_trait(Em.entity_trait.FLOATY_ITEM):
					velocity.y += FMath.percent(GRAVITY, 25)
				else:
					velocity.y += GRAVITY
					
				var terminal = FMath.percent(GRAVITY, TERMINAL_VELOCITY_MOD)
				if UniqEntity.has_trait(Em.entity_trait.FLOATY_ITEM):
					terminal = FMath.percent(terminal, 25)
...
# warning-ignore:narrowing_conversion
	if is_against_wall(sign(orig_vel_x), soft_dbox):
		...
		if UniqEntity.has_trait(Em.entity_trait.NO_BOUNCE):
			velocity.y = 0
