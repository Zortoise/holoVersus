...
func play_random_in_folder(folder_path: String, return_dict := false):
	
	if last_folder_path == folder_path: return
	last_folder_path = folder_path
	
	var dir_name = OS.get_executable_path().get_base_dir() + "/GameMusic/" + folder_path + "/"
	var dir = Directory.new()
	
	var music_name_list := []
	
	if dir.dir_exists(dir_name): # if folder exist
		if dir.open(dir_name) == OK:
			dir.list_dir_begin(true)
			var file_name = dir.get_next()
			while file_name != "":
				if file_name.ends_with(".ogg"):
					music_name_list.append(file_name.trim_suffix(".ogg"))
				file_name = dir.get_next()
		else:
			print("Error: Cannot open " + folder_path + " folder")
			return
	else:
		print(folder_path + " folder not found")
		return
			
	if music_name_list.size() == 0: 
		print("No OGG files in " + folder_path + " folder")
		return
		
	music_name_list.shuffle()
	var file_name = music_name_list[0] # get filename of random song
	var dictionary = {
			"name" : file_name, # default song name
			"vol" : 0,
		}
	var file = File.new()
	file.open(dir_name + file_name + ".ogg", file.READ) # load the .ogg
	var buffer = file.get_buffer(file.get_len())
	var new_stream = AudioStreamOGGVorbis.new()
	new_stream.data = buffer
	dictionary["stream"] = new_stream
	
	var tres_name = dir_name + file_name + ".tres" # load the .tres
	if ResourceLoader.exists(tres_name):
		var track_data = ResourceLoader.load(tres_name).data
		for key in track_data.keys():
			dictionary[key] = track_data[key]
	...
	bgm(dictionary)
	if return_dict: return dictionary
	
	
func check_custom():
	var dir_name = OS.get_executable_path().get_base_dir() + "/GameMusic/CustomPlaylist/"
	var dir = Directory.new()
	...
	if dir.dir_exists(dir_name): # if folder exist
		if dir.open(dir_name) == OK:
			...
			while file_name != "":
				if file_name.ends_with(".ogg"):
					has_custom = true # found at least 1 OGG file in CustomPlaylist
					return
		else:
			print("Error: Cannot open CustomPlaylist folder")
			return
	else:
		print("CustomPlaylist folder not found")
		return
...
#				if file_name.ends_with(".ogg"):
#					var dictionary = {
#						"name" : file_name.trim_suffix(".ogg"),
#						"vol" : 0,
#					}
#
#					var file = File.new()
