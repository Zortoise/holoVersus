		...
		Cards.effect_ref.FUWA_SLASH:
			if !Cards.effect_ref.FUWA_SLASH in enhance_cooldowns:
				...
				if target != null and target != self:
					...
					if !skip_cooldown:
						enhance_cooldowns[Cards.effect_ref.FUWA_SLASH] = Cards.FWMC_COOLDOWN
		Cards.effect_ref.MOCO_SLASH:
			if !Cards.effect_ref.MOCO_SLASH in enhance_cooldowns:
				...
				if target != null and target != self:
					...
					if !skip_cooldown:
						enhance_cooldowns[Cards.effect_ref.MOCO_SLASH] = Cards.FWMC_COOLDOWN
						
		Cards.effect_ref.SCISSORS:
			if !Cards.effect_ref.SCISSORS in enhance_cooldowns:
				var target = get_target()
				if target != null and target != self:
					Globals.Game.spawn_entity(player_ID, "ScissorsE", target.position, {"target_ID":target.player_ID})
					play_audio("cut3", {"vol" : -20})
					if !skip_cooldown:
						enhance_cooldowns[Cards.effect_ref.SCISSORS] = Cards.SCISSORS_COOLDOWN
...
func dodge_enhance():
	...
	if Inventory.has_quirk(player_ID, Cards.effect_ref.SCISSORS):
		enhance_card(Cards.effect_ref.SCISSORS)
...
func can_guardcrash(hit_data) -> bool: # check if collision boxes are close enough
	...
	if Em.hit.NPC_PATH in hit_data:
		return false # NPCs cannot Guardcrash
