...
#	Entity.play_audio("cut6", {"vol" : -5})
	if "dual" in aux_data:
		Entity.unique_data["count"] = 3
		Entity.face(-Entity.facing)
	elif "count" in aux_data:
...
func simulate():
	...
	if Animator.time == 8 and "count" in Entity.unique_data and Entity.unique_data.count > 0:
		Globals.Game.spawn_entity(Entity.master_ID, "MocoSlashE", Entity.position, {"facing" : -Entity.facing, "count" : Entity.unique_data.count})
